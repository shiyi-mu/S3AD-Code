...
def test_mono_detection(data, module:nn.Module,
    ...
    scores, bbox, obj_index, _, _ = module(
...
def test_stereo_detection(data, module:nn.Module,
    ...
    with torch.no_grad():
        scores, bbox, obj_index, _, _ = module([left_images.cuda().float().contiguous(), right_images.cuda().float().contiguous(), torch.tensor(P2).cuda().float(), torch.tensor(P3).cuda().float()])
    obj_types = [cfg.obj_types_fg[i.item()] for i in obj_index]
    return scores, bbox, obj_types
