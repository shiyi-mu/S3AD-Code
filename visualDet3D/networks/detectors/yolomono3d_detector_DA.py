...
from visualDet3D.networks.detectors.yolomono3d_core_depth_anything import YoloMono3DCoreDA
...
class GroundAwareHead_DA(AnchorBasedDetection3DHead):
...
class Yolo3D_DA(nn.Module):
    """
        YoloMono3DNetwork
    """
    def __init__(self, network_cfg):
        super(Yolo3D_DA, self).__init__()
    ...
    def build_core(self, network_cfg):
        self.core = YoloMono3DCoreDA(network_cfg.backbone)
    ...
    def training_forward(self, img_batch, annotations, P2):
        ...
        cls_loss, fg_loss, reg_loss, loss_dict = self.bbox_head.loss(cls_preds, reg_preds, anchors, annotations, P2)
    ...
    def test_forward(self, img_batch, P2):
        ...
        scores, bboxes, cls_indexes, ls_score_backup, anchor_backup  = self.bbox_head.get_bboxes(cls_preds, reg_preds, anchors, P2, img_batch)
...
@DETECTOR_DICT.register_module
class GroundAwareYolo3D_DA(Yolo3D_DA):
    """Some Information about GroundAwareYolo3D"""

    def build_head(self, network_cfg):
        self.bbox_head = GroundAwareHead_DA(
            **(network_cfg.head)
        )
