...
class SigmoidFocalLossFg(nn.Module):
    def __init__(self, gamma=0.0, balance_weights=torch.tensor([1.0], dtype=torch.float)):
        super(SigmoidFocalLossFg, self).__init__()
        self.gamma = gamma
        self.register_buffer("balance_weights_2d", balance_weights)
    ...
    def forward(self, classification:torch.Tensor, 
        ...
        """
            input:
                classification  :[..., 1]  linear output
                targets         :[..., num_classes] == -1(ignored), 0, 1
            return:
                foreground_loss        :[..., num_classes]  loss with 0 in trimmed or ignored indexes 
        """
        if gamma is None:
            gamma = self.gamma
        if balance_weights is None:
            balance_weights = self.balance_weights_2d
        # targets 转换 是否为前景的标签，num_classes的二元分类中，有任意1则为1
        # 沿着最后一个维度求最大值
        # print("targets min", torch.min(targets), "max", torch.max(targets))
        targets_fg, max_indices = torch.max(targets, dim=-1)
        # 对每一行检查是否有 1 或 -1
        # targets_fg = torch.where(torch.any(targets == 1, dim=1), torch.tensor(1).cuda(),
        #                     torch.where(torch.any(targets == -1, dim=1), torch.tensor(-1).cuda(), torch.tensor(0).cuda()))
        targets_fg = targets_fg.unsqueeze(-1)
        # print("targets_fg", targets_fg.shape)
        probs = torch.sigmoid(classification) #[B, N, 1]
        focal_weight = torch.where(torch.eq(targets_fg, 1.), 1. - probs, probs)
        focal_weight = torch.pow(focal_weight, gamma)

        bce = -(targets_fg * nn.functional.logsigmoid(classification)) * balance_weights - ((1-targets_fg) * nn.functional.logsigmoid(-classification)) #[B, N, 1]
        # print("bce", bce.shape)
        cls_fg_loss = focal_weight * bce

        ## neglect  0.3 < iou < 0.4 anchors
        cls_fg_loss = torch.where(torch.ne(targets_fg, -1.0), cls_fg_loss, torch.zeros(cls_fg_loss.shape).cuda())

        ## clamp over confidence and correct ones to prevent overfitting
        cls_fg_loss = torch.where(torch.lt(cls_fg_loss, 1e-5), torch.zeros(cls_fg_loss.shape).cuda(), cls_fg_loss) #0.02**2 * log(0.98) = 8e-6
