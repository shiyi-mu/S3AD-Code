...
from visualDet3D.networks.heads.losses import SigmoidFocalLoss, SigmoidFocalLossFg, ModifiedSmoothL1Loss
...
class AnchorBasedDetection3DHead(nn.Module):
    def __init__(self, num_features_in:int=1024,
        ...
        self.read_precompute_anchor = read_precompute_anchor
        ...
        self.ood_score_type = ood_score_type
    ...
    def forward(self, inputs):
        ...
        fg_preds = self.fg_feature_extraction(inputs['PSV_features'])
        cls_preds = torch.cat([cls_preds, fg_preds], dim=-1)
    ...
    def build_loss(self, focal_loss_gamma=0.0, balance_weight=[0], L1_regression_alpha=9, **kwargs):
        ...
        self.loss_cls_3d = SigmoidFocalLoss(gamma=focal_loss_gamma, balance_weights=self.balance_weights)
        self.loss_cls_2d = SigmoidFocalLoss(gamma=focal_loss_gamma, balance_weights=self.balance_weights)
        self.loss_fg = SigmoidFocalLossFg(gamma=focal_loss_gamma, balance_weights=self.balance_weights)
    ...
    def _encode_2d(self, sampled_anchors, sampled_gt_bboxes):
        assert sampled_anchors.shape[0] == sampled_gt_bboxes.shape[0]
        ...
        stds = targets.new([0.1, 0.1, 0.2, 0.2])
        targets = targets.div_(stds)
        return targets
    ...
    def _decode_2d(self, boxes, deltas):
        std = torch.tensor([0.1, 0.1, 0.2, 0.2], dtype=torch.float32, device=boxes.device)
        widths  = boxes[..., 2] - boxes[..., 0]
        heights = boxes[..., 3] - boxes[..., 1]
        ctr_x   = boxes[..., 0] + 0.5 * widths
        ctr_y   = boxes[..., 1] + 0.5 * heights
    ...
    def get_anchor(self, img_batch, P2):
        ...
        if self.read_precompute_anchor:
            anchors, useful_mask, anchor_mean_std = self.anchors(img_batch, P2, is_filtering=is_filtering)
        else:
            anchors, useful_mask = self.anchors(img_batch, P2, is_filtering=is_filtering)
            N = anchors.shape[1]
            anchor_means = torch.zeros(self.num_classes, N, 6)+0.001
            anchor_stds = torch.ones(self.num_classes, N, 6)
            anchor_mean_std = torch.stack([anchor_means, anchor_stds], dim=-1).permute(1, 0, 2, 3).cuda() #[N, types, 6, 2]
        return_dict=dict(
    ...
    def get_bboxes(self, cls_scores, reg_preds, anchors, P2s, img_batch=None):
        ...
        fg_score = cls_scores[0][..., self.num_classes+1:self.num_classes+2]
        ...
        fg_score = fg_score[useful_mask]
        # 前景减去已知类别即为ood得分
        
        cls_score_car = cls_score[:, 0:1]
        cls_score_cyc = cls_score[:, 1:2]
        cls_score_other = cls_score[:, 2:3]
        cls_score_ped= cls_score[:, 3:4]
        score_thr = getattr(self.test_cfg, 'score_thr', 0.5)
        # print("len(self.balance_weights)", len(self.balance_weights))
        # print("self.num_classes", self.num_classes)
        # print("self.score_thr", score_thr)
        if self.ood_score_type == "MSP":
            cls_score_norm = cls_score[:, 0:len(self.balance_weights)]
            # cls_score_norm = F.softmax(cls_score_norm, dim=-1)
            MSP, _ = torch.max(cls_score_norm, dim =-1)
            MSP = MSP.unsqueeze(1)
            fg_score = (fg_score > 0.5).float()
            ood_score_MSP = fg_score - MSP
            ood_score_MSP = torch.clamp(ood_score_MSP, min=0)
            ood_score = ood_score_MSP
        elif self.ood_score_type == "MSPF":
            cls_score_norm = cls_score[:, 0:len(self.balance_weights)]
            # cls_score_norm = F.softmax(cls_score_norm, dim=-1)
            MSP, _ = torch.max(cls_score_norm, dim =-1)
            MSP = MSP.unsqueeze(1)
            ood_score_MSPF = fg_score - MSP
            ood_score_MSPF = torch.clamp(ood_score_MSPF, min=0)
            ood_score = ood_score_MSPF
        # ood_score_RbMF = fg_score - cls_score[:, 0:len(self.balance_weights)].max(dim=-1)[0].unsqueeze(1)
        elif self.ood_score_type == "RBA":
            fg_score = (fg_score > 0.5).float()
            cls_score_norm = cls_score[:, 0:len(self.balance_weights)]
            Mean_norm = torch.mean(cls_score_norm, dim =-1)
            Mean_norm = Mean_norm.unsqueeze(1)
            ood_score_RbAF = fg_score - Mean_norm
            ood_score = ood_score_RbAF
       
        elif self.ood_score_type == "RBAF":
            ood_score_RbAF = fg_score - cls_score[:, 0:len(self.balance_weights)].sum(dim=-1).unsqueeze(1)/(len(self.balance_weights))
            ood_score = ood_score_RbAF

        else:
            print(">>>wrong ood sccore type")
            raise NotImplementedError


        # ood_score_RbA = 1 - cls_score.sum(dim=-1).unsqueeze(1)/cls_score.shape[-1]
        
        # ood_score = fg_score - (cls_score_car + cls_score_cyc + cls_score_ped)/3
        # ood_score = cls_score_other
        cls_score[:, len(self.balance_weights):] *= 0
        cls_score = torch.cat([cls_score[:,:-1], ood_score], dim=-1)
        # 记录anchor， cls_score备份
        anchor_backup = anchor.clone()
        cls_score_backup = cls_score.clone()
        fg_score_backup = fg_score.clone()
        cls_score_backup = torch.cat([cls_score_backup, fg_score_backup], dim=-1)
        ...
        # print("debug>>>mushiy>>>test_cfg = ", self.test_cfg)
        # print("debug>>>mushiy>>>score_thr = ", score_thr)
        ...
        return max_score, bboxes, label, cls_score_backup, anchor_backup
    
    def loss_depth_uncertainty(self, depth_input, target_depths, depth_log_variance):
        depth_log_variance = depth_log_variance.clamp(min=1e-4)
        depth_loss = 1.4142 * torch.exp(-depth_log_variance) * torch.abs(depth_input - target_depths) + depth_log_variance  
        return depth_loss
    
    def loss(self, cls_scores, reg_preds, anchors, annotations, P2):
        ...
        for j in range(batch_size):
            ...
            # fg_score = cls_scores[j][..., self.num_classes+1:self.num_classes+2]
            ...
            if self.depth_uncertainty:
                reg_pred = reg_pred[:, :-1]
                reg_depth_log_variance = reg_pred[:, -1:]
            cls_score = cls_score[useful_mask]
            # fg_score = fg_score[useful_mask]
            alpha_score = alpha_score[useful_mask]
            ...
            if len(bbox_annotation) == 0:
                ...
                fg_loss.append(torch.tensor(0).cuda().float())
                reg_loss.append(reg_preds.new_zeros(self.num_regression_loss_terms))
                depth_loss.append(reg_preds[:,:].new_zeros(1))
            ...
            if len(pos_inds) > 0:
                ...
                if len(selected_anchor_3d) > 0:
                    ...
                    if self.decode_before_loss:
                        ...
                        if self.depth_uncertainty:
                            depth_loss_j = self.loss_depth_uncertainty(pos_prediction_decoded[mask][:,6:7], 
                                                                       pos_target_decoded[mask][:,6:7], 
                                                                       reg_depth_log_variance[pos_inds][mask][:,0:1])
                            depth_loss.append(depth_loss_j.mean(dim=0))
                        ...
                        reg_loss.append(loss_j.mean(dim=0)) #[13]
                        if self.depth_uncertainty:
                            depth_loss_j = self.loss_depth_uncertainty(reg_pred[pos_inds][:,6:7], 
                                                                       pos_bbox_targets[:,6:7], 
                                                                       reg_depth_log_variance[pos_inds][:,0:1])
                            depth_loss.append(depth_loss_j.mean(dim=0))
                        number_of_positives.append(bbox_annotation.shape[0])
            else:
                reg_loss.append(reg_preds.new_zeros(self.num_regression_loss_terms))
            ...
            if len(neg_inds) > 0:
                labels[neg_inds, :] = 0
            # cls_loss.append(self.loss_cls_3d(cls_score, labels).sum() / (len(pos_inds) + len(neg_inds)))
            # fg_loss.append(self.loss_fg(fg_score, labels).sum() / (len(pos_inds) + len(neg_inds)))
            
        weights = reg_pred.new(number_of_positives).unsqueeze(1) #[B, 1]
        # cls_loss = torch.stack(cls_loss).mean(dim=0, keepdim=True)
        cls_loss =None
        # fg_loss = torch.stack(fg_loss).mean(dim=0, keepdim=True)
        fg_loss = None
        reg_loss = torch.stack(reg_loss, dim=0) #[B, 12]
        if self.depth_uncertainty is True:
           depth_loss = torch.stack(depth_loss, dim=0)
           reg_loss[:, 6:7] = depth_loss/12.0
        # reg_loss = reg_loss[:, 4:]
        ...
        return cls_loss, fg_loss, reg_loss, dict(cls_loss=cls_loss, 
                                                 fg_loss=fg_loss, 
                                                 reg_loss=reg_loss, 
                                                 total_loss=reg_loss)
    
    def loss_ext(self, cls_scores, reg_preds, anchors, annotations_ext, P2):
        batch_size = cls_scores.shape[0]
        ...
        for j in range(batch_size):
            ...
            anchor_j = anchor[useful_mask]
            anchor_mean_std_3d_j = anchor_mean_std_3d[useful_mask]
            reg_pred = reg_pred[useful_mask,0:4]
            cls_score = cls_score[useful_mask]
            fg_score = fg_score[useful_mask]
            alpha_score = alpha_score[useful_mask]

            # only select useful bbox_annotations
            bbox_annotation = annotations_ext[j, :, :]
            bbox_annotation = bbox_annotation[bbox_annotation[:, 4] != -1]#[k]
            ...
            labels = anchor_j.new_full((num_valid_anchors, self.num_classes),
                                    -1, # -1 not computed, binary for each class
                                    dtype=torch.float)
            # print("num_valid_anchors", num_valid_anchors)
            # print("pos_inds", len(sampling_result_dict['pos_inds']))
            # print("neg_inds", len(sampling_result_dict['neg_inds']))
            pos_inds = sampling_result_dict['pos_inds']
            neg_inds = sampling_result_dict['neg_inds']
            ...
            if len(pos_inds) > 0:
                ...
                if len(selected_anchor_3d) > 0:
                    ...
                    pos_gt_bboxes = sampling_result_dict['pos_gt_bboxes'][selected_mask]
                    pos_assigned_gt = sampling_result_dict['pos_assigned_gt_inds'][selected_mask]
                    pos_bbox_targets = self._encode_2d(
                        pos_bboxes, pos_gt_bboxes) #[N, 4], [N, 1]
                    label_index = pos_assigned_gt_label[selected_mask]
                    labels[pos_inds, :] = 0
                    labels[pos_inds, label_index] = 1 
                    ...
                    pos_alpha_score = alpha_score[pos_inds]
                    if self.decode_before_loss:
                        pos_prediction_decoded = self._decode_2d(pos_anchor, reg_pred[pos_inds])
                        pos_target_decoded = self._decode_2d(pos_anchor, pos_bbox_targets)
                        reg_loss_j = self.loss_bbox(pos_prediction_decoded, pos_target_decoded)
                        reg_loss.append(reg_loss_j.mean(dim=0)) #[4]
                        number_of_positives.append(bbox_annotation.shape[0])
                    else:
            ...
            cls_score_know = cls_score[:, 0: len(self.balance_weights)]
            label_know = labels[:, 0: len(self.balance_weights)]

            cls_loss.append(self.loss_cls_2d(cls_score_know, label_know).sum() / (len(pos_inds) + len(neg_inds)))
            # cls_loss.append(self.loss_cls_2d(cls_score, labels).sum() / (len(pos_inds) + len(neg_inds)))
            
            # fg_loss.append(self.loss_cls_2d(fg_score, labels).sum() / (len(pos_inds) + len(neg_inds)))
            # fg_loss.append(self.loss_cls_2d(cls_score, labels)[:, -1].sum() / (len(pos_inds) + len(neg_inds)))

            
            # self.loss_cls_2d(cls_score_know, labels_know)
            # cls_loss.append(self.loss_cls_2d(cls_score_know, labels_know).sum() / (len(pos_inds) + len(neg_inds)))
            fg_loss.append(self.loss_fg(fg_score, labels).sum() / (len(pos_inds) + len(neg_inds)))
        ...
        # cls_loss = None
        fg_loss = torch.stack(fg_loss).mean(dim=0, keepdim=True)
        # print("fg_loss[-1]", fg_loss)
        # print("cls_loss[-1]", cls_loss)
        reg_loss = torch.stack(reg_loss, dim=0) #[B, 4]
...
class StereoHead(AnchorBasedDetection3DHead):
    def init_layers(self, num_features_in,
        ...
        self.fg_feature_extraction = nn.Sequential(
            ConvBnReLU(1152, fg_feature_size, (3, 3)),
            BasicBlock(fg_feature_size, fg_feature_size),
            nn.ReLU(),
            nn.Conv2d(fg_feature_size, num_anchors*1, kernel_size=3, padding=1),
            AnchorFlatten(1)
        )
        self.fg_feature_extraction[-2].weight.data.fill_(0)
        self.fg_feature_extraction[-2].bias.data.fill_(0)
...
class StereoHead_Light(AnchorBasedDetection3DHead):
    def init_layers(self, num_features_in,
                          num_anchors:int,
                          num_cls_output:int,
                          num_reg_output:int,
        ...
        self.fg_feature_extraction = nn.Sequential(
            ConvBnReLU(512, fg_feature_size, (3, 3)),
            BasicBlock(fg_feature_size, fg_feature_size),
            nn.ReLU(),
            nn.Conv2d(fg_feature_size, num_anchors*1, kernel_size=3, padding=1),
            AnchorFlatten(1)
...
class StereoHead_fgByLeftStereo(AnchorBasedDetection3DHead):
    def init_layers(self, num_features_in,
                          num_anchors:int,
                          num_cls_output:int,
                          num_reg_output:int,
        ...
        self.fg_feature_extraction = nn.Sequential(
            ConvBnReLU(num_features_in, fg_feature_size, (3, 3)),
            BasicBlock(fg_feature_size, fg_feature_size),
            nn.ReLU(),
            nn.Conv2d(fg_feature_size, num_anchors*1, kernel_size=3, padding=1),
            AnchorFlatten(1)
    ...
    def forward(self, inputs):
        cls_preds = self.cls_feature_extraction(inputs['features'])
        reg_preds = self.reg_feature_extraction(inputs['features'])
        fg_preds = self.fg_feature_extraction(inputs['features'])
        cls_preds = torch.cat([cls_preds, fg_preds], dim=-1)
        return cls_preds, reg_preds



class StereoHead_fgByLeft(AnchorBasedDetection3DHead):
    def init_layers(self, num_features_in,
                          num_anchors:int,
                          num_cls_output:int,
                          num_reg_output:int,
        ...
        self.fg_feature_extraction = nn.Sequential(
            ConvBnReLU(256, fg_feature_size, (3, 3)),
            BasicBlock(fg_feature_size, fg_feature_size),
            nn.ReLU(),
            nn.Conv2d(fg_feature_size, num_anchors*1, kernel_size=3, padding=1),
            AnchorFlatten(1)
    ...
    def forward(self, inputs):
        cls_preds = self.cls_feature_extraction(inputs['features'])
        reg_preds = self.reg_feature_extraction(inputs['features'])
        fg_preds = self.fg_feature_extraction(inputs['left_features_final'])
        cls_preds = torch.cat([cls_preds, fg_preds], dim=-1)
        return cls_preds, reg_preds
