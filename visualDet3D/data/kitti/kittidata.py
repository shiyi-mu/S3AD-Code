...
class KittiObj():
    ...
    def __init__(self, s=None):
        ...
        if len(s.split()) == 15: # data
            ...
            print(">>>mushiyi>>>debug:", s.split(), len(s.split()))
...
class KittiData:
    ...
    def __init__(self, root_dir, idx, output_dict=None):
        ...
        self.label3_path = os.path.join(root_dir, "label_3_grddino", idx+'.txt')
    ...
    def replace_dir_prefix(self, ori_pre, new_pre):
        self.calib_path =  self.calib_path.replace(ori_pre, new_pre)
        self.image2_path = self.image2_path.replace(ori_pre, new_pre)
        self.image3_path = self.image3_path.replace(ori_pre, new_pre)
        self.label2_path = self.label2_path.replace(ori_pre, new_pre)
        self.label3_path = self.label3_path.replace(ori_pre, new_pre)
        self.velodyne_path = self.velodyne_path.replace(ori_pre, new_pre)
    ...
    def read_data(self):
        ...
        returns:
            ...
            pc(np.array): [# of points, 4]
                ...
                y<----.z
        '''
        # self.replace_dir_prefix("/data3/mushiyi/", "/server19/mushiyi/")   
        ...
        if os.path.exists(self.label3_path):
            label3 = KittiLabel(self.label3_path).read_label_file() if self.output_dict["label3"] else None
        else:
            # print("label3 path is ont exsit! {}\n".format(self.label3_path)*5)
            label3 = None
        ...
        if 'image_3' in self.output_dict and self.output_dict['image_3']:
            ...
            return calib, image, image_3, label, pc, label3
        else:
            return calib, image, label, pc, label3
