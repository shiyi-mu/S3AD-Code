...
def calculate_iou_partly(gt_annos,
    ...
    for num_part in split_parts:
        ...
        print("gt_annos_part", gt_annos_part)
        print("dt_annos_part", dt_annos_part)
        if metric == 0:
...
def eval_class(gt_annos,
    ...
    if len(gt_annos) != len(dt_annos):
        print('Errors: num of gt{} != num of dt{}.'.format(len(gt_annos), len(dt_annos)))
    assert len(gt_annos) == len(dt_annos)
    num_examples = len(gt_annos)
    num_parts=1
    split_parts = get_split_parts(num_examples, num_parts)
        ...
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    ...
                    print("tp, fp, fn, similarity, thresholds", tp, fp, fn, similarity, thresholds)
...
def get_official_eval_result(gt_annos,
    ...
    """
    overlap_mod = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.7, 0.7, 0.7, 0.7],
                            [0.7, 0.5, 0.5, 0.7, 0.5, 0.7, 0.7, 0.7, 0.7],
                            [0.7, 0.5, 0.5, 0.7, 0.5, 0.7, 0.7, 0.7, 0.7]])
    overlap_easy = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5],
                             [0.5, 0.25, 0.25, 0.5, 0.25, 0.5, 0.5, 0.5, 0.5],
                             [0.5, 0.25, 0.25, 0.5, 0.25, 0.5, 0.5, 0.5, 0.5]])
    overlap_25 = np.array([[0.25, 0.25, 0.25, 0.7, 0.5, 0.5, 0.5, 0.5, 0.25],
                           [0.25, 0.25, 0.25, 0.5, 0.25, 0.5, 0.5, 0.5, 0.25],
                           [0.25, 0.25, 0.25, 0.5, 0.25, 0.5, 0.5, 0.5, 0.25]])
    min_overlaps = np.stack([overlap_mod, overlap_easy, overlap_25], axis=0)  # [2, 3, 5]
    class_to_name = {
        0: 'Car',
        1: 'Pedestrian',
        2: 'Cyclist',
    ...
    current_classes_int = []
    current_classes = ["Ood"]
    print("mushiyi>>>> current_classes", current_classes)
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            print("metrics 3d precision", metrics["3d"]["precision"])
            print("i = ", i)
            print("mAP3d j, :, i", metrics["3d"]["precision"][j, :, i])
