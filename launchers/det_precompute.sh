...
elif [ $SPLIT == "val_only" ]; then
    echo -e "Precomputation for the val_only split"
    python3 scripts/imdb_precompute_3d.py --config=$CONFIG_PATH --split=$SPLIT
elif [ $SPLIT == "test" ]; then
    echo -e "Precomputation for the test split"
    python3 scripts/imdb_precompute_test.py --config=$CONFIG_PATH
else
    echo -e "split must in (train, val_only, test)" 
fi
