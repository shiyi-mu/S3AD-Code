...
cfg.obj_types_kitti = ['Car', 'Cyclist']    # train for classification loss
cfg.obj_types = cfg.obj_types_kitti # train for 3d+2d loss
cfg.obj_types_fg = cfg.obj_types + ["person", "Ood"] # for 2d fg loss
...
cfg.obj_types_test_Ood = ['parking_meter', 'polar_bear', 'horse', 'bulldog', 'mailbox_(at_home)', 
                          ...
                          'statue_(sculpture)',"person", "Pedestrian"]  # will be mapped to "Ood" for test

cfg.obj_types_test_Ood += ['OtherForeground'] # will be mapped to "Ood"
cfg.obj_types_test = ['Car', 'Cyclist', 'Ood'] # for test result
...
path.data_path = "smb9_msy/03-Data/02-KITTI/object/kitti_stereo_GDINO"
...
path.project_path = os.path.join(path.project_path, '07-A3D-for-paper-r1', "201-ped-eval-stereo")
...
train_split_file = "smb9_msy/03-Data/02-KITTI/object/kitti_stereo_GDINO/ImageSets/train.txt" #GDINO
...
head_loss = edict(
    ...
    balance_weight = [20.0, 40.0], # 长度等于obj_types_kitti长度
